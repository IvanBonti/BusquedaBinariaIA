...
def combined_heuristic(a, b):
    manhattan = heuristic_manhattan(a, b)
    euclidean = heuristic_euclidean(a, b)
    return (manhattan + euclidean) / 2  # Combina las dos heurísticas

def astar(grid, start, goal):
    open_list = []
    closed_list = set()
    
    start_node = Node(start, 0, combined_heuristic(start, goal))
    goal_node = Node(goal)
    ...
    while open_list:
        ...
        for next_pos in neighbors:
            if (0 <= next_pos[0] < len(grid) and
                ...
                h = combined_heuristic(next_pos, goal_node.position)
...
def main():
    ...
    pygame.display.set_caption('A* Pathfinding with Combined Heuristic')
